...
struct SnowID
{
	...
	constexpr SnowID(const SnowID& rhs)
		: hiPart(rhs.hiPart), loPart(rhs.loPart)
	{
	}

	constexpr SnowID(const uint64_t& hi, const uint64_t& lo)
		: hiPart(hi), loPart(lo)
	{
	}

	constexpr static SnowID Construct(uint32_t d1, uint16_t d2, uint16_t d3,
	...
	constexpr static SnowID Null()
	{
		return SnowID(0, 0);
	}

	constexpr bool IsNull() const { return hiPart == 0 && loPart == 0; }
	constexpr bool operator==(const SnowID& rhs) const { return hiPart == rhs.hiPart && loPart == rhs.loPart; }
	constexpr bool operator!=(const SnowID& rhs) const { return hiPart != rhs.hiPart || loPart != rhs.loPart; }
	constexpr bool operator<(const SnowID& rhs) const { return hiPart < rhs.hiPart || ((hiPart == rhs.hiPart) && loPart < rhs.loPart); }
	constexpr bool operator>(const SnowID& rhs) const { return hiPart > rhs.hiPart || ((hiPart == rhs.hiPart) && loPart > rhs.loPart); }
	constexpr bool operator<=(const SnowID& rhs) const { return hiPart < rhs.hiPart || ((hiPart == rhs.hiPart) && loPart <= rhs.loPart); }
	constexpr bool operator>=(const SnowID& rhs) const { return hiPart > rhs.hiPart || ((hiPart == rhs.hiPart) && loPart >= rhs.loPart); }
	...
	constexpr static SnowID FromStringInternal(const char* string)
	{
		return SnowID::Construct
		(
...
constexpr SnowID operator"" _guid(const char* input, size_t)
{
	return (input[0] == '{') ? SnowID::FromStringInternal(input + 1) : SnowID::FromStringInternal(input);
}
...
namespace std
{
	...
	struct hash<SnowID>
	{
		size_t operator()(const SnowID& guid) const
		{
